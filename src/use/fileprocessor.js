...
FileProcessor.prototype.replaceWithRevved = function replaceWithRevved(lines, assetSearchPath) {
  ...
  regexps.forEach(function (rxl) {
    ...
    content = content.replace(rxl[0], function (match, src) {
      ...
      //取消手动打上的版本号
      function cancelHand(text){
        if(text.indexOf("?v=")!=-1){
          return text.substring(0,text.indexOf("?v="));
        }else{
          return text
        }
      }
      ...
      //${styleLocation}
      //变量
      var reg=/\$\{(stylePath|scriptLocation)}\//;
      var placeText="${stylePath}/";

      var jspFlag=srcFile.indexOf("${")!=-1;
      var srcFile= cancelHand(srcFile.replace(reg,""));
      ...
      if(res.indexOf("link")!=-1&&jspFlag){
        var tempArr= res.split('href="');
        console.log(res)
        return tempArr[0]+'href="'+placeText+tempArr[1];
      }if(res.indexOf("script")!=-1&&jspFlag){
        var tempArr= res.split('src="');
        return tempArr[0]+'src="'+placeText+tempArr[1];
      }else{
        return res;
      }
    });
